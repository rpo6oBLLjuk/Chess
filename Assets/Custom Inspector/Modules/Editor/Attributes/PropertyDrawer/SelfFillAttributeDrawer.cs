            ...
            else
            {
                ...
                else //property.objectReferenceValue != null
                {
                    ...
                    if (property.objectReferenceValue is GameObject g)
                    {
                        ...
                        {
                            if (Application.isPlaying)
                                Debug.LogError($"SelfFill: GameObject reference value on '{property.name}' deleted. SelfFillAttribute only valid for gameObject on '{sa.mode}'. Location: '{property.serializedObject.targetObject}'");
                            property.objectReferenceValue = null;
                            property.serializedObject.ApplyModifiedProperties();
                        }
                    }
                    else if (property.objectReferenceValue is Component c)
                    {
                        ...
                        {
                            Debug.LogWarning($"SelfFill: Value on '{property.name}' had wrong type. '{requiredInterface.requiredType}' is not assignable from {c.GetType()}.\nValue set to null");
                            property.objectReferenceValue = null;
                            property.serializedObject.ApplyModifiedProperties();
                        }
                        ...
                        {
                            if (Application.isPlaying)
                                Debug.LogError($"SelfFill: ObjectReferenceValue on '{property.name}' deleted. SelfFillAttribute only valid for components on '{sa.mode}'. Location: '{property.serializedObject.targetObject}'");
                            property.objectReferenceValue = null;
                            property.serializedObject.ApplyModifiedProperties();
                        }
                    }
                    ...
                    {
                        if (Application.isPlaying)
                            Debug.LogError($"SelfFill: Reference on '{property.name}' deleted, because selffillattribute only supports components. Location: '{property.serializedObject.targetObject}'");
                        else
                            Debug.LogWarning($"SelfFill: Value on '{property.name}' discarded, because selffillattribute only supports components");
                        property.objectReferenceValue = null;
                        property.serializedObject.ApplyModifiedProperties();
                    }
                }
